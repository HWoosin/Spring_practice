...
public class FreeBoardControllerTest {
	...
	@Test
	@DisplayName("3번 글 상세 보기 요청을 넣으면 컨트로러는 DB에서 가지고 온 글 객체를"
			+ "model에 담아서 jsp로 이동")
	void testContent() throws Exception {
		...
		assertEquals("freeboard/content", mv.getViewName());
	...
	void testModify() throws Exception {
		String updatePage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/modify")
				.param("title","테스트 새 글 제목아님ㅋ")
				.param("content", "테스트 새 글 내용아니라고ㅋㅋ")
				.param("bno","6"))
				.andReturn().getModelAndView().getViewName();
		System.out.println("viewName: "+updatePage);
	}
	...
	void testDelete() throws Exception {
		String delcontent = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/delete")
				.param("bno","3"))
				.andReturn().getModelAndView().getViewName();
		assertEquals("redirect:/freeboard/freeList ",delcontent);
	}
