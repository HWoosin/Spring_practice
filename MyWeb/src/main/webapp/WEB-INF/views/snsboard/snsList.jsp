...
<body>
	...
	<script>
		...
		function regist() {
			...
			FormData 객체를 활용합니다.
			무조건 이 방식이 옳은 것은 아니다! FormData는 비동기 방식이 꼭 필요한 경우에만 사용.
			대부분의 경우에는 form 태그를 이용해서 전송하는 방식이 더 간편하고 더 자주 사용됨.
			*/
			...
			//한 화면에 파일 업로드 벼튼이 여러개일 경우 요소의 인덱스를 지목해서 가져올 수 있다.
			//우리는 파일 첨부버튼이 하나고, id를 지목해서 가져오기 때문에 인덱스를 쓸 필요는 없다.
			//console.log('data[0]' + $data[0]);
			console.log($data.files); //파일 태그에 담긴 파일 정보를 확인하는 프로퍼티.
			console.log($data.files[0]);
			//사용자가 등록한 최종 파일의 정보
			//만약 우리가 여러파일을 하나의 태그로 받을 수 있도록 multiple을 제공했다면
			...
			//FormData 객체를 보낼 때는 따로 header설정을 진행하지 않는다.
		...
		/*
		무한스크롤 페이징
		모든 게시판이 무한스크롤페이징 방식이 어울리는 것은 아니다.
		...
		*/
		...
		window.onscroll = function(){
			...
			문서 높이 - 브라우저창 높이 = 스크롤 창의 끝 높이와 같다면 -> 새로운 내용 불러오기
			*/
			if(window.innerHeight + window.scrollY >=document.body.scrollHeight){
				//사용자의 스크롤이 바닥에 닿았을 때, 페이지 변수의 값을 하나 올리고
				//reset여부는 false를 주셔서 누적해서 계속 불러오면 된다.
				//게시글 한번에 몇개씩 불러올지는 PageVo의 cpp를 조정하면된다.
				console.log('페이징 발동');
				getList(++page, false);
			}
			}
			else{
				console.log('더이상 불러올 목록이 없어요!');
			}
