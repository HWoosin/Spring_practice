...
public class SnsBoardController {
	...
	public String delete(@PathVariable int bno, HttpSession session){
		...
		return file.delete() ? "success" : "fail";//삭제하면 success	
	}
	...
	snsList.jsp 페이지가 로딩 되면서, 글 목록을 가져오고 있고, JS를 이용해서 화면을 꾸밀 때
	img 태그의 src에 작성된 요청 url을 통해 자동으로 요청이 들어오게 됩니다.
	요청을 받아주는 메서드를 선언하여 경로에 지정된 파일을 보낼 예정.
	*/
	...
	public ResponseEntity<byte[]> getFile(@PathVariable String fileLoca,
		...
		//응답 내용, 응답이 성공했는지에 대한 여부, 응답에 관련된 여러가지 설정들을 지원.
		ResponseEntity<byte[]> result = null;
